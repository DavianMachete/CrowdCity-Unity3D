...
public class CharacterCollisionController : MonoBehaviour
{
    ...
    private List<CharacterController> opponentAndFreeCharacters;
    private List<Crowd> opponetsCrowd;

    public void Prepare(CharacterController controller, float seekingDistance, int checkingCountInAFrame)
    {
        if (opponentAndFreeCharacters == null)
            opponentAndFreeCharacters = new List<CharacterController>();
